...
DataCoverage <- sapply(VarNames, function(x) length(na.omit(Data[,x]))/length(Data[,x]) )
...
# C8 First probit regressions - three best variables with respecto to AUC
...
#another idea - don't remove outliers but set limits
# C14 setting limits
...
NewRegress2 <- glm(formula=Formula,na.action=na.exclude,family=binomial("probit"), data = NewData2)
NewRegress2$coefficients
summary(NewRegress2)

NewFittedScores2 <- predict.glm(NewRegress2)
NewPred2 <- prediction(NewFittedScores2,NewData2$deflag)
NewAUC2 <- as.numeric(performance(NewPred2,"auc")@y.values)
